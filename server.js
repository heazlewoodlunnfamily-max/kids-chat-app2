...
// HTML - Perfect with bubble tea and iPhone notifications
const html = `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, viewport-fit=cover">
    <title>🧋 Bubble Tea Chat</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .container { width: 100%; max-width: 480px; background: white; border-radius: 28px; box-shadow: 0 20px 60px rgba(0,0,0,0.3); display: none; flex-direction: column; height: 90vh; max-height: 750px; overflow: hidden; padding-top: max(0px, env(safe-area-inset-top)); }
        .container.show { display: flex; }
        
        .header { background: linear-gradient(90deg, #FF6B9D 0%, #FFA06B 25%, #FFE66D 50%, #95E1D3 75%, #667eea 100%); color: white; padding: 16px; border-radius: 0; display: flex; justify-content: space-between; align-items: center; font-size: 20px; font-weight: 700; padding-top: max(16px, calc(16px + env(safe-area-inset-top))); }
        ...
        .chat-display { flex: 1; overflow-y: auto; padding: 16px; background: 
            linear-gradient(135deg, #fff9f0 0%, #fef5e7 50%, #fff9f0 100%),
            repeating-linear-gradient(45deg, transparent, transparent 35px, rgba(255, 107, 157, 0.03) 35px, rgba(255, 107, 157, 0.03) 70px),
            repeating-linear-gradient(-45deg, transparent, transparent 35px, rgba(167, 230, 207, 0.03) 35px, rgba(167, 230, 207, 0.03) 70px),
            radial-gradient(circle at 15% 85%, rgba(255, 183, 77, 0.08) 0%, transparent 50%),
            radial-gradient(circle at 85% 15%, rgba(102, 126, 234, 0.08) 0%, transparent 50%);
            background-attachment: fixed;
        }
        .chat-display::-webkit-scrollbar { width: 6px; }
        .chat-display::-webkit-scrollbar-track { background: transparent; }
        .chat-display::-webkit-scrollbar-thumb { background: #ddd; border-radius: 3px; }
        ...
        .input-area { padding: 12px; padding-bottom: max(12px, calc(12px + env(safe-area-inset-bottom))); background: white; border-top: 2px solid #e8eaf6; display: flex; flex-direction: column; gap: 8px; }
        ...
        .compose-send { background: linear-gradient(90deg, #FF6B9D 0%, #FFA06B 25%, #FFE66D 50%, #95E1D3 75%, #667eea 100%); color: white; }
        ...
        .input-field { flex: 1; min-width: 80px; padding: 11px 14px; border: 2px solid #e0e0e0; border-radius: 20px; font-size: 14px; font-family: inherit; font-weight: 500; transition: all 0.3s; }
        ...
        .btn-compose { background: linear-gradient(90deg, #FF6B9D 0%, #FFA06B 25%, #FFE66D 50%, #95E1D3 75%, #667eea 100%); color: white; box-shadow: 0 3px 10px rgba(156, 39, 176, 0.3); }
        ...
        .send-btn { background: linear-gradient(90deg, #FF6B9D 0%, #FFA06B 25%, #FFE66D 50%, #95E1D3 75%, #667eea 100%); color: white; border: none; padding: 10px 14px; border-radius: 20px; font-size: 12px; font-weight: 700; cursor: pointer; transition: all 0.3s; box-shadow: 0 3px 10px rgba(102, 126, 234, 0.3); text-transform: uppercase; letter-spacing: 0.2px; white-space: nowrap; flex-shrink: 0; }
    ...
    <script>
            ...
            ws.onmessage = (e) => {
                ...
                if (data.type === 'load_messages') {
                    ...
                    if (data.data.user !== currentUser) {
                        playSound();
                        showNotification(data.data.user, data.data.text);
                    }
        ...
        function updateTimeLock() {
            ...
            if (locked) {
                ...
                msgInput.placeholder = 'Chat locked 7pm-7am';
            } else {
        ...
        function send() {
            if (isAppLocked()) {
                alert('⏰ Chat is locked between 7pm-7am');
                return;
            }
